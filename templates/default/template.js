...
function numericMasking(){
	...
	$("#survey-wrapper").delegate("input.formatnumeric","keyup", function(){ 
	...
	// Remove the last dot
	$("#survey-wrapper").delegate("input.formatnumeric","focusout", function(){ 
	...
	if($('.maskFlag').length > 0) {
		...
		$('.maskFlag').each(function(i){
			...
			if(maskName=="integeronly" || maskName=="numeric")
			{
				$('input[type=text]', el).each(function(){
					...
					newinput="<input type='text' class='text formatnumeric "+integeronly+emsqvalidation+"' rel='"+thisid+"' id='new"+thisid+"' "+maxlen+" size='"+$(this).attr('size')+"' value='"+$(this).attr('value')+"' />";
		...
		$('input.maskednumeric').keyup(function(e){
...
function adaptTable(){
  var doremove=true;
  if($("body").hasClass("allinone")){ 
    if($("#showfirstqgrid,#showallqgrid").length>0){
      doremove=false;
      }
    }// We can not do it with survey in one page: because we don't know if we need all colummn and line.
    $(".survey-groupdescription table,.survey-question-text table").each(function(index){
      ...
      if($(this).find("col").length==0)
      {
        ...
        $(this).find("thead tr td,thead tr th").children(index).each(function(){
...
// This part can wrok, but think it's best to use only relevace equation
    ...
    // Show Hidden group
    $("#showgroup").live("click",function() {
        $("[id^=group-]").show();
        $("#showgroup").remove();
    });
    // Show hidden question, not in table and not equation (?)
    //$("[id^=group-] > .question-wrapper:not(.equation)").show();
    // And now for gridly table cell

    $("#showfirstqgrid").live("click",function() {
        ...
        $("#showfirstqgrid").remove();
    });
    $("#showallqgrid").live("click",function() {
        $(".survey-groupdescription table th .question-wrapper").show();
        $(".survey-groupdescription table td .question-wrapper").show();
        $("#showallqgrid").remove();
    });
    $("#showallq").live("click",function() {
        $("[id^=group-] > .question-wrapper:not(.equation)").show();
        $("#showallq").remove();
    });
}
function input2text()
{
  $(".survey-groupdescription table,table.summary").find('input.text:visible').each(function() {
    $(this).addClass("dontprint");
    $(this).after($("<span class='toprint' />").text(this.value));
  });
}
